...
export const QuizService = {
  ...
  async submitAttempt(userId: string, payload: QuizAttemptPayload) {
    const {
      categoryId,
      timeTaken,
      answers,
    ...
    if (answers && answers.length > 0) {
      const questionMap = new Map(
        category.questions.map((q: any) => [String(q._id), q])
      );

      answerRecords = answers.map((ans: AnswerPayload) => {
  ...
  async getLeaderboard(query: Record<string, unknown>) {
    const { page, limit, skip } = parsePaginationQuery(query);
